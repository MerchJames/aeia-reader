...
/**
 * A theme that brings its own per-word arrival has to bring the CSS for it.
 *
 * Calligraphy is the case: the script face is half the idea and `quill` is the
 * other half — a fancy font that fades in reads as a fancy font, the same font
 ...
 */
for (const id of Object.keys(THEMES) as Theme[]) {
  const fx = THEMES[id].streamEffect;
  if (!fx) continue;
  ok(css.includes(`.word-reveal-${fx}`), `${id}: "${fx}" has a word reveal to run`);
}
